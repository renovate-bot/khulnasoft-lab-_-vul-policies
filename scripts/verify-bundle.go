...
func createVulContainer(ctx context.Context, regIP string) testcontainers.Container {
	...
	vulC, err := testcontainers.GenericContainer(ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: reqVul,
		Started:          true,
	})
	if err != nil {
		panic(err)
...
func LoadBundle() {
	...
	vulC := createVulContainer(ctx, regIP)
	defer func() {
		if err = vulC.Terminate(ctx); err != nil {
			panic(err)
		}
	}()
	...
	fmt.Println(debugLogsForContainer(ctx, vulC))
}
